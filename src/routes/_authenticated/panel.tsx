...
import { AlertTriangle, ArrowRight, ClipboardList, Clock } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { CATEGORIAS, formatoFecha } from "@/lib/catep";
        ...
        <div className="space-y-2">
          ...
          {(resumen?.recientes ?? []).map((r) => (
            <div key={r.id} className="card-elevated flex items-center gap-3 p-3">
              <span
                className={`size-2.5 shrink-0 rounded-full ${r.eventualidad ? "bg-destructive" : "bg-success"}`}
              />
              <span className="min-w-0 flex-1">
...
function Metrica({
  ...
  return (
    <div className="card-elevated flex flex-col gap-1 p-3">
      <Icono className={`size-4 ${destacar ? "text-destructive" : "text-primary"}`} />
